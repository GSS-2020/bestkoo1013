...
let humanDifficulty = 'medium'; // 'easy', 'medium', 'hard' for human vs human mode
...
function playSound(type) {
    ...
    try {
        ...
        switch (type) {
            case 'place':
                // Dot placement sound - soft click
...
// Update sound control buttons
function updateSoundButtons() {
    const soundBtn = document.querySelector('.sound-toggle');
    
    if (soundBtn) {
        soundBtn.innerHTML = soundEnabled ? '🔊' : '🔇';
        soundBtn.title = soundEnabled ? 'Matikan Bunyi' : 'Hidupkan Bunyi';
    }
}
...
function selectGameMode(mode) {
    ...
    if (mode === 'human') {
        ...
        document.getElementById('human-difficulty-section').style.display = 'block';
        ...
        document.getElementById('human-difficulty-section').style.display = 'none';
...
// Select Human vs Human difficulty
function selectHumanDifficulty(difficulty) {
    humanDifficulty = difficulty;
    
    // Update button states for human difficulty
    document.getElementById('human-easy-btn').classList.remove('active');
    document.getElementById('human-medium-btn').classList.remove('active');
    document.getElementById('human-hard-btn').classList.remove('active');
    
    document.getElementById('human-' + difficulty + '-btn').classList.add('active');
    
    console.log('Human difficulty selected:', difficulty);
}
...
function placeDot(row, col, element) {
    ...
    // If next player is AI, make AI move immediately
    if (isAIPlayer2 && currentPlayer === 2 && gameActive) {
        setTimeout(() => {
            makeAIMove();
        }, 500); // Quick delay for better user experience
    }
}
...
function updatePlayerTurn() {
    ...
    // Update full screen background if in full screen mode
    updateFullScreenBackground();
}

// Function to sync background in full screen mode
function updateFullScreenBackground() {
    if (isFullScreen) {
        const container = document.querySelector('.container');
        const bodyStyle = window.getComputedStyle(document.body);
        if (container) {
            container.style.background = bodyStyle.background;
...
function updateScores() {
    ...
    // Get winning capture count based on difficulty
    let winningCaptureCount = 10;
    
    if (isAIPlayer2) {
        // AI mode difficulty
        switch(aiDifficulty) {
            case 'easy':
                winningCaptureCount = 10;
                break;
            case 'medium':
                winningCaptureCount = 12;
                break;
            case 'hard':
                winningCaptureCount = 14;
                break;
        }
    } else {
        // Human vs Human mode difficulty
        switch(humanDifficulty) {
            case 'easy':
                winningCaptureCount = 10;
                break;
            case 'medium':
    ...
    if (player1Captures > player2Captures) {
        leadingIndicator.innerHTML = `🏆 ${player1Name} Memimpin dengan ${player1Captures - player2Captures} kepungan<br><small>Sasaran: ${winningCaptureCount} kepungan</small>`;
        leadingIndicator.style.color = '#2c3e50';
        score1Element.classList.add('leading');
    } else if (player2Captures > player1Captures) {
        leadingIndicator.innerHTML = `🏆 ${player2Name} Memimpin dengan ${player2Captures - player1Captures} kepungan<br><small>Sasaran: ${winningCaptureCount} kepungan</small>`;
        leadingIndicator.style.color = '#e74c3c';
        score2Element.classList.add('leading');
    } else {
        leadingIndicator.innerHTML = `⚖️ Seri - Tiada pemimpin<br><small>Sasaran: ${winningCaptureCount} kepungan</small>`;
...
function checkWinCondition() {
    // Dynamic winning capture count based on difficulty
    let winningCaptureCount = 10; // Default
    
    if (isAIPlayer2) {
        // AI mode difficulty
        switch(aiDifficulty) {
            case 'easy':
                winningCaptureCount = 10; // Easy: default 10 captures
                break;
            case 'medium':
                winningCaptureCount = 12; // Medium: +2 captures (12 total)
                break;
            case 'hard':
                winningCaptureCount = 14; // Hard: +4 captures (14 total)
                break;
        }
    } else {
        // Human vs Human mode difficulty
...
function endGame(winType = 'normal') {
    ...
    if (player1Captures > player2Captures) {
        ...
        if (winType === 'capture') {
            winReason = `Mencapai ${player1Captures} kepungan! (Sasaran: ${winningCaptureCount})`;
        } else {
        ...
        if (winType === 'capture') {
            winReason = `Mencapai ${player2Captures} kepungan! (Sasaran: ${winningCaptureCount})`;
        } else {
    ...
    let difficultyInfo = '';
    if (isAIPlayer2) {
        difficultyInfo = `<p><strong>Mod AI ${aiDifficulty.toUpperCase()}:</strong> Sasaran kemenangan ${winningCaptureCount} kepungan</p><br>`;
    } else {
        difficultyInfo = `<p><strong>Mod ${humanDifficulty.toUpperCase()}:</strong> Sasaran kemenangan ${winningCaptureCount} kepungan</p><br>`;
    }
...
function makeAIMove() {
    ...
    setTimeout(() => {
        if (!gameActive || currentPlayer !== 2 || !isAIPlayer2) {
            aiThinking = false;
            return;
        }
        ...
        if (move) {
            ...
            if (element && gameActive && currentPlayer === 2) {
                aiThinking = false;
                placeDot(move.row, move.col, element);
            }
        } else {
            // No valid moves available, end AI thinking
            aiThinking = false;
            updatePlayerTurn();
            
            // Check if game should end due to no moves
            checkGameState();
        }
    }, 300 + Math.random() * 700); // Faster AI response
}
...
// Easy AI - Random valid moves, but avoid obvious traps
function getEasyAIMove() {
    ...
    for (let row = 1; row <= 9; row++) {
        for (let col = 1; col <= 9; col++) {
            if (gameBoard[row][col] === 0) {
                ...
                // Check if it's not in enemy trap
    ...
    // Prefer safe moves, but if none available, use any valid move
...
function enterFullScreen() {
    ...
    // Sync background with current player turn
    updateFullScreenBackground();
    ...
    newGameBtn.className = 'fullscreen-new-game';
    newGameBtn.innerHTML = '🎮 Permainan Baru';
    newGameBtn.onclick = startNewGame;
    newGameBtn.title = 'Mulai permainan baru';
...
function exitFullScreen() {
    ...
    const newGameBtn = document.querySelector('.fullscreen-new-game');
    if (newGameBtn) {
        newGameBtn.remove();
    }
